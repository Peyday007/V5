-- The Postgres half of server/db/migrations/029_conversation_lanes.sql. The two
-- chains are numbered independently; what they must agree about is the shape.
--
-- One deliberate difference, and it is the same one every other pair has: each
-- table carries an explicit `seq` identity column, because SQLite's `rowid` is
-- free and Postgres's is not. `dialect.ts` rewrites `rowid` to `seq`, so a
-- table without it turns an ordering into a missing-column error — which is
-- precisely how this file was found to be wrong, by running the suite against
-- a real Postgres rather than by reading it.
-- ---------------------------------------------------------------------------
...
CREATE TABLE llm_models (
  -- SQLite gives every row a hidden insertion counter and thirty-odd queries
  -- use `rowid` as the tiebreaker that makes `ORDER BY created_at, rowid` an
  -- order. `dialect.ts` translates that word to `seq`, so every table the two
  -- chains share needs this column or the translation names nothing.
