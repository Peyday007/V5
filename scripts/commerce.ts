/**
 * The social commerce kernel's operator surface, on a terminal.
 *
 ...
 *
 *   npm run commerce -- --project cash-mode-3 --channel "TikTok Shop" --admin you@example.com
 *   npm run commerce -- --project cash-mode-3                (read and advance)
 *   npm run commerce -- --project cash-mode-3 --read         (read only; writes nothing)
 *
 ...
 *      what can be prepared, raise what is missing. `--read` skips it, because
 *      somebody reading the loop's state should be able to do that without
 *      moving it: `services/realize/prove.ts` splits reading from applying for
 *      the same reason.
 ...
 *
 * The last line is the verdict, at column zero, and `commerce.yml` greps for
 * it: an exit code has to survive an SSH session, a shell and a CLI, and a
 * line the command printed does not.
 */
...
import { planFrom, runCommerceKernel, snapshot } from '../server/services/commerce/kernel.ts';
...
async function main(): Promise<void> {
  ...
  const named = arg('project');
  if (!named) {
    console.log('Pass --project <slug|id>. Projects that run a sprint:');
    for (const project of await listProjects()) {
      const mode = await getCashMode(project.id);
      if (mode) console.log(`  ${project.id}  ${project.slug}  ${project.name}  [${mode.state}]`);
    }
    return;
  }

  // Named by slug or by id, because the id is what a machine holds and the
  // slug is what a person types — and neither should be the only one that works.
  const project = (await getProjectBySlug(named)) ?? (await getProject(named));
  ...
  const readOnly = has('read');
  const channelName = arg('channel');
  if (readOnly && channelName) {
    throw new Error('--read writes nothing, so it cannot also seed a channel. Pick one.');
  }
  if (channelName) {
    /*
  ...
  if (readOnly) {
    // `planFrom` over a `snapshot` is what the pass would do, with nothing
    // written — so a person can ask what Brain would ask next without asking it.
    const state = await snapshot(projectId);
    const plan = planFrom(state);
    console.log('');
    console.log(`WOULD ASK — ${plan.asks.length} round(s), nothing written`);
    for (const ask of plan.asks) {
      console.log(`  ${ask.purpose} about ${trim(ask.subject, 80)}`);
      console.log(`    because: ${trim(ask.why, 160)}`);
    }
  ...
  for (const channel of view.channels) {
    for (const term of channel.terms) {
      console.log(`      term ${term.kind} [${term.basis}] ${term.figure ?? '—'} — ${trim(term.statement, 100)}`);
    }
  }

  /*
   * The ranked list, with the four things the directive insists stay apart:
   * what was *watched*, what somebody is shown to have *bought*, a margin
   * *estimated* from published figures, and one *measured* by a test of ours.
   * They are four separate readings on the row, so they are printed as four —
   * collapsing them into a score is exactly what this kernel refuses to do.
   */
  for (const [index, piece] of view.best.entries()) {
    console.log(
      `  ${index + 1}. ${trim(piece.product, 60)} [${piece.stage}] ` +
        `purchase=${piece.purchaseReadings} attention=${piece.attentionReadings}`,
    );
    console.log(
      `      contribution: ${
        piece.contribution
          ? `${piece.contribution.minor} minor [${piece.contribution.basis}]`
          : `withheld — ${trim(piece.contributionWithheld, 160)}`
      }`,
    );
    if (piece.unknown.length > 0) {
      console.log(`      unknown: ${piece.unknown.join(', ')}`);
    }
    console.log(`      next: ${trim(piece.next.what, 140)} (${piece.next.owner})`);
    if (piece.next.blockedBy) console.log(`      blocked by: ${piece.next.blockedBy}`);
    for (const would of piece.wouldChange) console.log(`      would change: ${trim(would, 140)}`);
  }
  for (const test of view.tests) {
    console.log(
      `  test ${test.id} ${test.state} ceiling=${test.ceilingMinor} blocker=${test.blocker ?? 'none'}`,
    );
    if (test.blockerDetail) console.log(`      ${trim(test.blockerDetail, 240)}`);
  }
  for (const asking of view.asking) {
    console.log(`  asking now: ${asking.purpose} about ${asking.subject}  opened=${asking.openedAt}`);
  }
  for (const next of view.next) {
    console.log(`  would ask next: ${next.purpose} about ${next.subject} — ${trim(next.why, 160)}`);
  }
  console.log('');
  console.log(
    `COMMERCE: project=${projectId} sprint=${mode.state} channels=${view.maturity.channels} ` +
      ...
      `purchase=${view.maturity.withPurchase} attention_only=${view.maturity.attentionOnly} ` +
      `margin=${view.maturity.withDerivableMargin} measured=${view.maturity.withMeasuredEvidence} ` +
